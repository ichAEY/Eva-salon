...
# Hero: user-selected category base files are treated as representative images.
...
# Build a varied seven-photo preview, then append every uploaded work by category.
preview=[]
...
s=re.sub(r'works\.slice\(0,\d+\)', 'works.slice(0,3)', s, count=1)
s=re.sub(r'works\.slice\(\d+,\d+\)', 'works.slice(3,7)', s, count=1)
s=s.replace('Открыть всю галерею','Смотреть все работы')

# Real team photo, while keeping master cards interactive.
if team:
...
# Remove remaining stale STLuxe location/details in the active EVA mobile layer.
...
# Desktop fallback should never show the old downloaded Yandex placeholder.
